 ...
 *   - Advanced Analytics (Red+)
 *   - AI Price Optimizer  (Red+)
 *   - AI Sales Predictor  (Red+)
 *   - AI Description Generator (Red+)
 *   - AI Recommender      (Red+)
 *
 * Uses the same fetch + token pattern as sellerApi.ts.
 */
...
export interface ProductAnalytics {
  products: Array<{
    ...
    views: number; is_active: boolean; is_approved: boolean;
    category_name: string;
    total_revenue: number; total_units: number; total_orders: number;
    avg_order_val: number; conversion_rate: number; revenue_per_view: number;
  }>;
...
export interface CustomerAnalytics {
  customers: Array<{
    ...
    last_order_at: string; days_since_last: number;
    rfm_score: number; segment: string;
  }>;
...
export const analyticsApi = {
  overview:  () =>
    jsonRequest<{ success: boolean; data: AnalyticsOverview }>('GET', '/seller/analytics/overview'),

  products:  () =>
    jsonRequest<{ success: boolean; data: ProductAnalytics }>('GET', '/seller/analytics/products'),

  customers: () =>
    jsonRequest<{ success: boolean; data: CustomerAnalytics }>('GET', '/seller/analytics/customers'),

  heatmap:   () =>
    jsonRequest<{ success: boolean; data: HeatmapData }>('GET', '/seller/analytics/heatmap'),
};
...
export const sellerAiApi = {
  priceOptimizer: (productId: number) =>
    jsonRequest<AIResponse<PriceOptimizerResult>>('POST', '/seller/ai/price-optimizer', { product_id: productId }),
