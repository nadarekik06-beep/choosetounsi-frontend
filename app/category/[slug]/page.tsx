 ...
 * ChooseTounsi — Redesigned Category Page
 * Design: Premium Trendyol-inspired with brand colors #db142e / #198f41
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Add to Cart Button (inline, no import)
// ─────────────────────────────────────────────────────────────────────────────
...
function CartBtn({ productId, stock }: { productId: number; stock: number }) {
  ...
  return (
    <button className={`ct-cart-btn ${oos ? 'ct-cart-btn--oos' : ''} ${state === 'done' ? 'ct-cart-btn--done' : ''}`} onClick={handle} disabled={oos || state === 'loading'}>
...
function WishBtn({ productId }: { productId: number }) {
  ...
  return (
    <button
      className={`ct-wish-btn ${active ? 'ct-wish-btn--on' : ''}`}
      onClick={e => { e.preventDefault(); e.stopPropagation(); setActive(v => !v) }}
      aria-label="Wishlist"
    >
    ...
    >
      ...
      <div className="ct-card__img-wrap">
        {imgSrc && !imgErr ? (
          ...
          <div className="ct-card__no-img">
            <svg width="36" height="36" fill="none" stroke="#d1d5db" strokeWidth="1.4" viewBox="0 0 24 24">
        ...
        {/* Quick view on hover */}
        <div className="ct-card__hover-bar">
          <CartBtn productId={product.id} stock={product.stock} />
        </div>
      </div>
      ...
      <div className="ct-card__body">
        ...
        {product.stock > 0 && product.stock < 10 && (
          <p className="ct-card__low-stock">Only {product.stock} left!</p>
        )}
      </div>
    </Link>
  )
...
interface FilterState {
  search: string
  priceMin: string   // empty = no filter applied
  priceMax: string   // empty = no filter applied
  inStock: boolean
  sort: SortKey
}

function Sidebar({
  filters, setFilters, total, mobileOpen, setMobileOpen
}: {
  filters: FilterState
  ...
  const upd = (partial: Partial<FilterState>) => setFilters({ ...filters, ...partial })

  const hasActiveFilters = !!(filters.search || filters.inStock || filters.priceMin || filters.priceMax)
    ...
    <aside className="ct-sidebar">
      ...
      <div className="ct-filter-section">
        ...
        <div className="ct-filter-sort-list">
          {SORT_OPTIONS.map(opt => (
            <button
              key={opt.key}
              className={`ct-sort-option ${filters.sort === opt.key ? 'ct-sort-option--active' : ''}`}
              onClick={() => upd({ sort: opt.key })}
            >
              <span>{opt.icon}</span>
              <span>{opt.label}</span>
              {filters.sort === opt.key && (
                <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" style={{ marginLeft: 'auto' }}><path d="M20 6L9 17l-5-5" /></svg>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="ct-filter-section">
        ...
        <div className="ct-price-inputs">
          ...
          />
          <button
            className="ct-price-go"
            title="Apply price filter"
            onClick={() => {/* filter is reactive */}}
          >
...
export default function CategoryPage() {
  ...
  // Client-side filter products — price only filters when user has typed a value
  ...
  }, [products, filters.search, filters.inStock, filters.priceMin, filters.priceMax])
    ...
    <>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .ct-animate { animation: ct-fade-up 0.45s ease both; }
        ...
        /* ── HERO BANNER ── */
        .ct-hero {
          background: linear-gradient(135deg, #fff5f5 0%, #fef2f2 40%, #fff 100%);
        ...
        .ct-hero__icon-wrap {
          ...
          font-size: 2rem;
          flex-shrink: 0;
          box-shadow: 0 8px 24px rgba(219, 20, 46, 0.3);
        }
        .ct-hero__eyebrow {
          font-size: 11px;
          font-weight: 700;
        ...
        .ct-hero__bg-shape {
          ...
          height: 300px;
          border-radius: 50%;
          background: rgba(219, 20, 46, 0.04);
          pointer-events: none;
        }
        .ct-hero__bg-shape2 {
          position: absolute;
          ...
          height: 200px;
          border-radius: 50%;
          background: rgba(25, 143, 65, 0.04);
          pointer-events: none;
        }
        ...
        .ct-card {
          ...
          flex-direction: column;
          text-decoration: none;
          animation: ct-fade-up 0.42s ease both;
        ...
        .ct-cart-btn {
          ...
          border: none; border-radius: 10px;
          cursor: pointer;
          box-shadow: 0 4px 16px rgba(219, 20, 46, 0.4);
        ...
        .ct-review-count {
          font-size: 10px; color: #9ca3af; font-weight: 500; margin-left: 3px;
        }
        ...
        .ct-skeleton__img {
          ...
          animation: ct-shimmer 1.3s infinite linear;
        }
        .ct-skeleton__body {
          padding: 12px 14px 14px;
        ...
        .ct-pagination {
          display: flex; align-items: center; justify-content: center;
          gap: 6px; padding: 32px 0 0; flex-wrap: wrap;
        }
        .ct-page-btn {
        ...
        <div className="ct-hero">
          ...
          <div className="ct-hero__inner">
            <div className="ct-hero__left" style={{ opacity: mounted ? 1 : 0, transform: mounted ? 'none' : 'translateY(12px)', transition: 'opacity 0.5s ease, transform 0.5s ease' }}>
              {category?.icon && (
                <div className="ct-hero__icon-wrap">{category.icon}</div>
              )}
              <div>
                ...
                {category?.description && <p className="ct-hero__desc">{category.description}</p>}
              </div>
            </div>
          ...
          <div className="ct-toolbar__inner">
            <div className="ct-toolbar__left">
              ...
              {SORT_OPTIONS.map(opt => (
                <button
                  key={opt.key}
                  className={`ct-sort-pill ${filters.sort === opt.key ? 'ct-sort-pill--active' : ''}`}
            ...
            <div className="ct-toolbar__right">
              {products && (
                <span className="ct-toolbar__result-count">
                  {filteredProducts.length} of {products.total}
                </span>
              )}
              {/* View mode */}
