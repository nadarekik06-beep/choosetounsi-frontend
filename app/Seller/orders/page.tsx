            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div style={{ background: bgSub, borderRadius: 14, padding: 16, border: `1px solid ${border}` }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <select value={newStatus} onChange={e => { setNewStatus(e.target.value); setSuccessMsg(''); }}
                    style={selectStyle}>
              ...
              <div style={{ background: bgSub, borderRadius: 14, padding: 16, border: `1px solid ${detail.order.payment_status === 'unpaid' ? '#f59e0b40' : border}` }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  {detail.order.payment_status === 'unpaid' && (
                    <span style={{ fontSize: 10, fontWeight: 700, color: '#f59e0b', background: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 6, padding: '2px 8px' }}>
                      💰 Cash on Delivery — confirm when cash received
                    </span>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                  <select value={newPayment} onChange={e => { setNewPayment(e.target.value); setSuccessMsg(''); }}
                    style={selectStyle}>
                  ...
                  <button onClick={handleUpdatePayment} disabled={!newPayment || updatingPayment}
                    style={{
                      padding: '8px 18px',
