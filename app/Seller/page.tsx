...
import {
  ...
  Star, Eye, BarChart2, CheckCircle, XCircle,
} from 'lucide-react';
...
function TopProductCard({ product, rank, dark }: { product: TopProduct; rank: number; dark: boolean }) {
  ...
  const imgSrc = product.primary_image_url
    ? (imgErr ? null : product.primary_image_url)
    : null;

  const rankColors = ['#f59e0b', '#94a3b8', '#f97316'];
  const rankColor  = rankColors[rank - 1] ?? '#64748b';

  const maxSales = product.total_sales > 0 ? product.total_sales : 1;

  return (
    <div className="top-product-card" style={{
      ...
      <div style={{
        ...
        background: rank <= 3 ? rankColor : '#64748b',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: 10, fontWeight: 900, color: '#fff',
        boxShadow: rank <= 3 ? `0 2px 8px ${rankColor}60` : 'none',
      }}>
      ...
      <div style={{ position: 'absolute', top: 10, right: 10, display: 'flex', gap: 4, zIndex: 2 }}>
        <span style={{
          ...
          background: product.is_active ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)',
          color: product.is_active ? '#10b981' : '#ef4444',
          border: `1px solid ${product.is_active ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'}`,
        }}>
          {product.is_active ? 'Active' : 'Off'}
        </span>
        <span style={{
          fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 999,
          background: product.is_approved ? 'rgba(59,130,246,0.15)' : 'rgba(245,158,11,0.15)',
          color: product.is_approved ? '#3b82f6' : '#f59e0b',
          border: `1px solid ${product.is_approved ? 'rgba(59,130,246,0.3)' : 'rgba(245,158,11,0.3)'}`,
        }}>
      ...
      <div style={{ minWidth: 0 }}>
        <p style={{ fontSize: 13, fontWeight: 800, color: textMain, margin: '0 0 2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {product.name}
        </p>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 15, fontWeight: 900, color: '#db142e', letterSpacing: '-0.01em' }}>
          {fmt(product.price)}
        </span>
        <span style={{
      ...
      <div style={{
        ...
        {[
          ...
          <div key={label} style={{
            background: subBg, borderRadius: 8, padding: '7px 6px',
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: textMuted }}>Orders</span>
          <span style={{ fontSize: 10, fontWeight: 800, color: textMain }}>{product.total_orders}</span>
        </div>
        <div style={{ height: 4, background: dark ? 'rgba(255,255,255,0.08)' : '#e5e7eb', borderRadius: 999, overflow: 'hidden' }}>
          <div style={{
            ...
            width: `${Math.min((product.total_orders / Math.max(product.total_orders, 1)) * 100, 100)}%`,
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '50vh' }}>
      <div style={{ textAlign: 'center', maxWidth: 340 }}>
        ...
        <p style={{ fontSize: 13, color: textMuted, margin: '0 0 18px' }}>
          ...
          <code style={{ background: 'rgba(255,255,255,0.08)', padding: '1px 5px', borderRadius: 5, fontSize: 11 }}>localhost:8000</code>.
        </p>
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 12 }}>
        {[...Array(6)].map((_, i) => <Skeleton key={i} dark={dark} style={{ height: 280 }} />)}
      </div>
    </div>
  );
        ...
        <div className={`fade-up ${visible?'show':''}`}>
          ...
          {top_products.length === 0 ? (
            <div style={{ background: cardBg, borderRadius: 18, border: `1px solid ${border}`, padding: '48px 20px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 11, color: textMuted, opacity: 0.6, margin: 0 }}>Sales stats will appear here once orders come in</p>
            </div>
          ) : (
            ...
            {top_clients.length === 0
              ...
              : top_clients.map((c, i) => (
                <div key={c.id} className="client-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', borderBottom: `1px solid ${border}`, transition: 'background 0.15s ease' }}>
                  <span style={{ width: 24, height: 24, borderRadius: '50%', background: i===0?'rgba(245,158,11,0.2)':i===1?'rgba(148,163,184,0.2)':i===2?'rgba(249,115,22,0.2)':'rgba(100,116,139,0.12)', color: i===0?'#f59e0b':i===1?'#94a3b8':i===2?'#f97316':'#64748b', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, flexShrink: 0 }}>
                    {i+1}
                  </span>
