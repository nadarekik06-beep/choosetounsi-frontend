 ...
 *
 * Unified Deals page:
 * - Flash Sales + Bundles in ONE grid, flash-first by default
 * - Category-page-style sidebar (Deal Type + Sort + Price + Availability)
 * - Flash cards: pulsing badge, countdown bar inside card, stock bar
 * - Bundle cards: existing PackCard logic, same 3/4 ratio
 * - 5-col grid, client-side filter/sort, single fetch on mount
 * - Mobile: sticky filter bar + drawer sidebar
 */
      ...
      <div className="dc-stage">
        {img && !imgErr ? (
          <img
            ...
            className={`dc-img${hov ? ' zoom' : ''}`}
            onError={() => setErr(true)}
            draggable={false}
          />
        ) : (
          <div className="dc-noimg">
            <svg width="32" height="32" fill="none" stroke="#ccc" strokeWidth="1.2" viewBox="0 0 24 24">
        ...
        {/* Badges — flash specific */}
        <div className="dc-badges">
          {item._discPct > 0 && (
            <span className="dc-badge dc-disc dc-disc-flash">-{item._discPct}%</span>
          )}
          <span className="dc-badge dc-flash-live">
            <span className="dc-pulse" />⚡ LIVE
          </span>
        </div>

        {/* Urgent overlay */}
        {isUrgent && (
          <div className="dc-urgent">🔥 Only {remaining} left!</div>
        )}

        {/* CTA — slides up on hover, pushes countdown up */}
        ...
        {/* ── Countdown bar — full width, dark strip at bottom ── */}
      ...
      {hasStock && (
        <div className="dc-stock-wrap">
          <div className="dc-stock-track">
            <div
              className={`dc-stock-fill${stockPct > 70 ? ' hot' : ''}`}
              style={{ width: `${Math.max(5, stockPct)}%` }}
            />
          </div>
          <span className={`dc-stock-txt${stockPct > 70 ? ' hot' : ''}`}>
            {remaining != null ? `${remaining} left` : 'Limited'}
          </span>
        </div>
      )}
...
function BundleCard({ item, idx }: { item: BundleItem; idx: number }) {
  ...
  const gallery = useMemo<string[]>(() => {
    ...
    pack.items.forEach(i => {
      const t = resolveUrl(i.product?.primary_image_url)
    ...
    >
      ...
      <div className="dc-stage">
        {slide && prev !== null && gallery[prev] && (
          <div className="dc-layer dc-layer-out">
            <img src={gallery[prev]} alt="" className="dc-img" draggable={false} />
          </div>
        )}
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gridTemplateRows:'1fr 1fr', width:'100%', height:'100%', gap:2 }}>
              {[0,1,2,3].map(i => (
                <div key={i} style={{ background:'#f1f5f9', overflow:'hidden' }}>
                  {thumbs[i] && <img src={thumbs[i]} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }} />}
                </div>
              ))}
            </div>
          ) : gallery[cur] && !imgErr ? (
        ...
        <div className="dc-badges">
          {item._discPct > 0 && (
            <span className="dc-badge dc-disc">-{item._discPct}%</span>
          )}
          <span className="dc-badge dc-bundle-tag">
        ...
        {gallery.length > 1 && (
          <div className="dc-dots">
            {gallery.slice(0, 5).map((_, i) => (
              <span key={i} className={`dc-dot${i === cur ? ' on' : ''}`} />
            ))}
          </div>
        )}
        ...
        <div className={`dc-cta${hov ? ' show' : ''}`}>
          <button
            className="dc-cta-btn dc-cta-bundle"
            onClick={e => { e.preventDefault(); e.stopPropagation(); router.push(`/deals/${pack.slug}`) }}
          >
            <svg width="11" height="11" fill="none" stroke="#fff" strokeWidth="2.2" viewBox="0 0 24 24">
              <path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/>
              <line x1="3" y1="6" x2="21" y2="6"/>
      ...
      <div className="dc-info">
        ...
        <div className="dc-prices">
          ...
          {pack.original_price > pack.pack_price && (
            <span className="dc-orig">{money(pack.original_price)}</span>
          )}
        </div>
        {pack.savings > 0 && (
          <p className="dc-save">Save {money(pack.savings)}</p>
        )}
      </div>
    </Link>
  )
...
const FlashSkel = () => (
  <div className="dsk">
    <div className="dsk-img" />
    <div className="dsk-bar" />
    <div className="dsk-body">
      <div className="dsk-ln" style={{ width:'40%', height:9 }} />
      <div className="dsk-ln" style={{ width:'72%', height:12, marginTop:5 }} />
      <div className="dsk-ln" style={{ width:'44%', height:14, marginTop:5 }} />
    </div>
  </div>
)
const BundleSkel = () => (
...
function DealsSidebar({
  f, setF, total, mOpen, setMOpen,
}: {
  f: Filters; setF: (v: Filters) => void
  total: number; mOpen: boolean; setMOpen: (v: boolean) => void
}) {
  const [open, setOpen] = useState(new Set(['sort', 'dealtype', 'price', 'avail']))
  const upd = (p: Partial<Filters>) => setF({ ...f, ...p })

  const tog = (k: string) => setOpen(s => {
    const n = new Set(s); n.has(k) ? n.delete(k) : n.add(k); return n
  })

  const isR = (mn: string, mx: string) => f.priceMin === mn && f.priceMax === mx
  const applyR = (mn: string, mx: string) =>
    isR(mn, mx) ? upd({ priceMin: '', priceMax: '' }) : upd({ priceMin: mn, priceMax: mx })
  ...
  const Acc = ({ k, label }: { k: string; label: string }) => (
    <button className="sb-head" onClick={() => tog(k)}>
      ...
      <svg width="10" height="10" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"
        style={{ transform: open.has(k) ? 'rotate(180deg)' : 'none', transition: 'transform .2s', flexShrink: 0 }}>
        <path d="M6 9l6 6 6-6"/>
      </svg>
    </button>
  )
  ...
  const inner = (
    <aside className="sbar">
      ...
      <div className="sbar-hd">
        <div>
          <p className="sbar-title">Filters</p>
          <p className="sbar-count">{total.toLocaleString()} deals</p>
        </div>
        {hasAny && (
          <button className="sbar-clear" onClick={() => setF(DEFAULT_FILTERS)}>✕ Clear</button>
        )}
      </div>

      {/* Sort By */}
      <div className="sb-acc">
        <Acc k="sort" label="Sort By" />
        {open.has('sort') && (
          <div className="sb-body">
            {SORTS.map(s => (
              <button
                key={s.k}
                className={`sbar-sort${f.sort === s.k ? ' on' : ''}`}
                onClick={() => upd({ sort: s.k })}
              >
                <span className="sbar-dot" />{s.l}
                {f.sort === s.k && (
                  <svg width="9" height="9" fill="none" stroke="#db142e" strokeWidth="2.5"
                    viewBox="0 0 24 24" style={{ marginLeft: 'auto' }}>
      ...
      {/* Deal Type — NEW */}
      <div className="sb-acc">
        <Acc k="dealtype" label="Deal Type" />
        {open.has('dealtype') && (
          <div className="sb-body">
            {([
              { k: 'all',     l: '🔥 All Deals' },
              ...
              <button
                key={d.k}
                className={`sbar-sort${f.dealType === d.k ? ' on' : ''}`}
                onClick={() => upd({ dealType: d.k })}
              >
      ...
      {/* Price Range */}
      <div className="sb-acc">
        <Acc k="price" label="Price Range" />
        {open.has('price') && (
          <div className="sb-body">
            <div className="sbar-pr-row">
              <input
                type="number" placeholder="Min" value={f.priceMin}
                onChange={e => upd({ priceMin: e.target.value })} className="sbar-pin"
              />
              <span style={{ color:'#bbb', fontSize:12 }}>–</span>
              <input
                type="number" placeholder="Max" value={f.priceMax}
                onChange={e => upd({ priceMax: e.target.value })} className="sbar-pin"
              />
            </div>
            {PRICE_RANGES.map(r => (
              <button
                key={r.l}
                className={`sbar-pr${isR(r.mn, r.mx) ? ' on' : ''}`}
                onClick={() => applyR(r.mn, r.mx)}
              >
                {r.l}
              </button>
            ))}
      ...
      {/* Availability */}
      <div className="sb-acc">
        <Acc k="avail" label="Availability" />
        {open.has('avail') && (
          <div className="sb-body" style={{ padding:'6px 16px 12px' }}>
  ...
  return (
    <>
      ...
      {mOpen && (
        <>
          <div className="sbar-bd" onClick={() => setMOpen(false)} />
          <div className="sbar-drawer">{inner}</div>
        </>
      )}
    </>
  )
...
export default function DealsPage() {
  ...
  const mergedItems = useMemo<DealItem[]>(() => {
    const flashItems: FlashItem[] = rawFlash.flatMap(sale =>
      (sale.products ?? []).map(p => {
        const discPct = p.original_price > 0
          ? Math.round(((p.original_price - p.effective_price) / p.original_price) * 100)
          : 0
        return {
          _type:    'flash' as const,
          _id:      `flash-${sale.id}-${p.id}`,
          _discPct: discPct,
          _price:   p.effective_price,
          product:  p,
          sale,
        }
      })
    ...
    const bundleItems: BundleItem[] = rawBundles.map(pack => {
      ...
      return {
        ...
        _id:      `bundle-${pack.id}`,
        _discPct: discPct,
        _price:   pack.pack_price,
        pack,
      }
    })
    return [...flashItems, ...bundleItems]
  }, [rawFlash, rawBundles])
  ...
  const filteredItems = useMemo<DealItem[]>(() => {
    return mergedItems.filter(item => {
      ...
      // Price filter
      if (filters.priceMin !== '') {
        if (item._price < Number(filters.priceMin)) return false
      }
      if (filters.priceMax !== '') {
        if (item._price > Number(filters.priceMax)) return false
      }
      // Stock filter — flash: use product.stock, bundle: always in stock
      if (filters.inStock) {
        if (item._type === 'flash') {
  ...
  const sortedItems = useMemo<DealItem[]>(() => {
    ...
    switch (filters.sort) {
      case 'flash_first':
        // Flash items first (sorted by discount%), then bundles (sorted by discount%)
        return arr.sort((a, b) => {
          if (a._type !== b._type) return a._type === 'flash' ? -1 : 1
          return b._discPct - a._discPct
        })
      case 'savings_desc':
        return arr.sort((a, b) => b._discPct - a._discPct)
      case 'price_asc':
        return arr.sort((a, b) => a._price - b._price)
      case 'price_desc':
        return arr.sort((a, b) => b._price - a._price)
      case 'newest':
        // Flash sorted by ends_at (soonest first = most urgent), bundles by id desc
        return arr.sort((a, b) => {
          if (a._type === 'flash' && b._type === 'flash') {
            return new Date(a.sale.ends_at).getTime() - new Date(b.sale.ends_at).getTime()
          }
          if (a._type === 'bundle' && b._type === 'bundle') {
            return b.pack.id - a.pack.id
          }
          return a._type === 'flash' ? -1 : 1
        })
      default:
        return arr
    }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ════════════════════════════════════════ */
        .dp-hero {
          background: linear-gradient(135deg, #c0111f 0%, #7f1d1d 55%, #450a0a 100%);
        ...
        .dp-hero-title {
          font-size:clamp(1.5rem,3.5vw,2.3rem); font-weight:900;
          color:#fff; letter-spacing:-0.025em; line-height:1.1; margin:0 0 5px;
        }
        ...
        .dp-stat-icon { font-size:16px; margin-bottom:1px; }
        .dp-stat-val  { font-size:20px; font-weight:900; color:#fff; letter-spacing:-0.02em; line-height:1; }
        .dp-stat-lbl  { font-size:9.5px; color:rgba(255,255,255,0.6); font-weight:700; text-transform:uppercase; letter-spacing:0.07em; }
        ...
        /* ════════════════════════════════════════
           SIDEBAR (exact category page styles)
        ════════════════════════════════════════ */
        .sbar {
          ...
          overflow-y:auto; scrollbar-width:thin; scrollbar-color:#f0f0f0 transparent;
        }
        .sbar-hd {
          display:flex; align-items:center; justify-content:space-between;
          padding:13px 16px 9px; border-bottom:1px solid #f5f5f5;
        ...
        .sbar-clear {
          background:rgba(219,20,46,.08); border:none; border-radius:6px;
          padding:4px 8px; font-size:10px; font-weight:700; color:#db142e;
          cursor:pointer; font-family:'Outfit',sans-serif; transition:background .12s;
        }
        ...
        .sb-head {
          ...
          text-transform:uppercase; letter-spacing:.09em; color:#444; transition:color .12s;
        }
        .sb-head:hover { color:#db142e; }
        .sb-body { padding:3px 15px 10px; }
        .sbar-sort {
          display:flex; align-items:center; gap:7px; width:100%;
          padding:7px 7px; border-radius:6px; background:none; border:none;
          cursor:pointer; font-family:'Outfit',sans-serif; font-size:12.5px;
          font-weight:500; color:#555; text-align:left; transition:all .11s;
        }
        .sbar-sort:hover { background:#f8f8f8; color:#db142e; }
        .sbar-sort.on { background:rgba(219,20,46,.05); color:#db142e; font-weight:700; }
        .sbar-dot {
          width:5px; height:5px; border-radius:50%; border:1.5px solid currentColor;
          flex-shrink:0; transition:background .11s;
        }
        .sbar-sort.on .sbar-dot { background:#db142e; }
        .sbar-pr-row { display:flex; align-items:center; gap:5px; margin-bottom:8px; }
        .sbar-pin {
          flex:1; min-width:0; padding:6px 8px; border:1.5px solid #e5e7eb;
          border-radius:7px; font-size:12px; font-family:'Outfit',sans-serif;
          color:#111; background:#f8f8f8; outline:none;
        ...
        .sbar-pr::before {
          ...
          border:1.5px solid #d1d5db; flex-shrink:0; background:#fff; transition:all .11s;
        }
        ...
        .sbar-trow {
          display:flex; align-items:center; justify-content:space-between;
          font-size:12.5px; font-weight:500; color:#374151; cursor:pointer;
        }
        .sbar-tgl {
          width:35px; height:19px; border-radius:999px; background:#e5e7eb;
          position:relative; cursor:pointer; flex-shrink:0; transition:background .19s;
        }
        .sbar-tgl.on { background:#db142e; }
        .sbar-tgl-k {
          position:absolute; top:2px; left:2px; width:15px; height:15px;
          border-radius:50%; background:#fff; box-shadow:0 1px 4px rgba(0,0,0,.14);
          transition:transform .19s;
        }
        .sbar-tgl.on .sbar-tgl-k { transform:translateX(16px); }
        .sbar-apply {
          display:none; width:calc(100% - 30px); margin:11px 15px 15px;
          padding:10px; background:#db142e; color:#fff; font-weight:800;
          font-size:12.5px; border:none; border-radius:9px; cursor:pointer;
          font-family:'Outfit',sans-serif; align-items:center; justify-content:center;
        }
        .sbar-bd {
          position:fixed; inset:0; background:rgba(0,0,0,.42); z-index:200;
          animation:dpFadeIn .17s ease; backdrop-filter:blur(2px);
        }
        .sbar-drawer {
          position:fixed; left:0; top:0; bottom:0; z-index:201;
          width:282px; max-width:90vw; overflow-y:auto;
          background:#fff; box-shadow:4px 0 24px rgba(0,0,0,.12);
          animation:dpSlideIn .22s ease;
        }
        ...
        /* ════════════════════════════════════════
           UNIFIED DEAL CARD  .dc
        ════════════════════════════════════════ */
        .dc {
          background:#fff; border-radius:11px; border:1px solid #eee;
          overflow:hidden; display:flex; flex-direction:column;
          text-decoration:none; cursor:pointer;
          animation:dpFadeUp .42s ease both;
          animation-delay:var(--d,0s);
          transition:box-shadow .22s, transform .22s, border-color .2s;
          will-change:transform;
        }
        /* Flash hover: red border tint */
        .dc[data-type="flash"]:hover {
          box-shadow:0 10px 32px rgba(219,20,46,.15);
          border-color:rgba(219,20,46,.35);
          transform:translateY(-3px) scale(1.015);
        }
        /* Bundle hover: neutral elevation */
        .dc[data-type="bundle"]:hover {
          box-shadow:0 10px 32px rgba(0,0,0,.1);
          border-color:#d8d8d8;
          transform:translateY(-3px) scale(1.015);
        }

        /* Stage — 3/4 portrait, same as category */
        .dc-stage {
        ...
        .dc-noimg {
          ...
          display:flex; align-items:center; justify-content:center; background:#f4f4f6;
        }

        /* Slide layers (bundle only) */
        ...
        /* Discount badge — flash gets slightly bigger font */
        ...
        .dc-urgent {
          ...
          text-align:center; padding:3px 8px; letter-spacing:.03em; z-index:6;
        }

        /* CTA bar */
        .dc-cta {
          position:absolute; bottom:0; left:0; right:0;
          padding:0 9px 9px; z-index:7;
          transform:translateY(110%); opacity:0;
          transition:transform .3s cubic-bezier(.34,1.48,.64,1), opacity .22s;
        }
        .dc-cta.show { transform:translateY(0); opacity:1; }
        .dc-cta-btn {
          display:flex; align-items:center; justify-content:center; gap:5px;
          width:100%; padding:8px 10px;
          font-size:11.5px; font-weight:800; border:none; border-radius:7px;
          cursor:pointer; font-family:'Outfit',sans-serif;
          transition:background .13s, transform .11s; letter-spacing:.01em;
        }
        .dc-cta-flash {
          background:#db142e; color:#fff;
          box-shadow:0 4px 14px rgba(219,20,46,.4);
        }
        ...
        /* Countdown bar — dark strip at image bottom, hidden when CTA shows */
        .dc-countdown {
          position:absolute; bottom:0; left:0; right:0;
          background:rgba(0,0,0,0.8); backdrop-filter:blur(4px);
          display:flex; align-items:center; justify-content:center;
          padding:5px 8px; z-index:6;
        ...
        .dc-stock-wrap {
          ...
          padding:5px 10px 2px; background:#fff;
        }
        .dc-stock-track {
          flex:1; height:3.5px; background:#f1f5f9;
        ...
        .dc-name {
          font-size:12px; font-weight:600; color:#1f2937; line-height:1.38;
          display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden;
        }
        .dc[data-type="flash"]:hover .dc-name  { color:#db142e; }
        .dc[data-type="bundle"]:hover .dc-name { color:#db142e; }
        .dc-desc {
          font-size:10px; color:#9ca3af; line-height:1.35;
        ...
        .dc-save   {
          ...
          background:#f0fdf4; padding:1px 6px; border-radius:999px;
        ...
        .dp-empty {
          ...
          padding:72px 24px; background:#fff; border-radius:12px; border:1px solid #eee;
          text-align:center; gap:10px; grid-column:1/-1;
        }
        ...
        @media(max-width:960px) {
          ...
          .sbar-desk  { display:none; }
          .dp-mbar    { display:flex; }
          .dp-grid    { grid-template-columns:repeat(3,1fr); gap:10px; padding-top:14px; }
          .dp-hero-stats { grid-template-columns:1fr 1px 1fr; }
          .dp-stat-div:nth-child(6),.dp-stat-div:nth-child(4) { display:none; }
          .dp-stat:nth-child(7) { display:none; }
        }
          ...
          <div className="dp-hero-inner">
            <div className="dp-hero-top">
              <div>
                ...
                <p className="dp-hero-sub">Flash sales &amp; curated bundles from Tunisia's best sellers</p>
              </div>
            ...
            <div className="dp-hero-stats">
              ...
              <div className="dp-stat">
                ...
                <span className="dp-stat-val">{loading ? '—' : `${maxFlashDisc}%`}</span>
                <span className="dp-stat-lbl">Max Flash Off</span>
              </div>
              <div className="dp-stat-div" />
              <div className="dp-stat">
                <span className="dp-stat-icon">📦</span>
              ...
              <div className="dp-stat">
                ...
                <span className="dp-stat-lbl">Max Bundle Save</span>
              </div>
            </div>
          </div>
        </div>
            ...
            {loading ? (
              <div className="dp-grid">
                {Array.from({ length: 10 }).map((_, i) =>
                  i % 3 === 0 ? <FlashSkel key={i} /> : <BundleSkel key={i} />
                )}
              </div>
              ...
              <div className="dp-grid">
                <div className="dp-empty">
                  ...
                  <p style={{ fontSize: 12, color: '#94a3b8', maxWidth: 260, lineHeight: 1.6 }}>
                    Try adjusting your filters or clearing them to see all deals.
                  </p>
                  <button
                    onClick={() => setFilters(DEFAULT_FILTERS)}
                    style={{ marginTop: 4, padding: '8px 18px', background: '#db142e', color: '#fff', fontWeight: 800, fontSize: 12, border: 'none', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit' }}
                  >
                    Clear All Filters
                  </button>
              ...
              <>
                <div className="dp-grid">
                  {displayedItems.map((item, i) =>
                    item._type === 'flash'
                      ? <FlashCard key={item._id} item={item} idx={i} />
